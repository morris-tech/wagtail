...
class TreeQuerySet(MP_NodeQuerySet):
    ...
    def first_common_ancestor(self, include_self=False, strict=False):
        """
        Find the first ancestor that all nodes in this queryset have in common.
        For example, consider a node heirarchy of Pages like::
        ...
        This method tries to be efficient, but if you have millions of nodes
        scattered across your node tree, it will be slow.

        If `include_self` is True, the ancestor can be one of the nodes in the
        queryset:
        ...
        node is in the queryset and ``include_self`` is False, and when there
        are multiple node trees with no common root (a case Wagtail does not
